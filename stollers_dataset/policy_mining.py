...
def recursive_policy_mining(access_data, attributes, current_rule):
    ...
    # gini_list = [(compute_gini_index(access_data, i), i) for i in list(range(len(attributes)))]
    gini_list = []
...
access_data = []
with open(r"C:\Users\Sathya\OneDrive\Desktop\BTP\spring_sem\stollers_dataset\healthcare_access_decisions.txt", "r") as file:
    for line in file:
...
with open(r"C:\Users\Sathya\OneDrive\Desktop\BTP\spring_sem\stollers_dataset\rules_stollers.txt", "w") as file:
    for rule in policy:
        file.write(str(rule) + "\n")
    file.write("\n")
